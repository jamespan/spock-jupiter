...
public class ParallelParameterizedSpecRunner extends ParameterizedSpecRunner {
    ...
    private void runIterations(Iterator<?>[] iterators, int estimatedNumIterations) {
        ...
        List<Future<IterationContext>> futures = new ArrayList<>();
        ...
        while (haveNext(iterators)) {
            ...
            CompletableFuture<IterationContext> future = CompletableFuture.supplyAsync(() -> {
                IterationContext currentContext = contexts.get(idx);

                ErrorInfo error = invoke(self, currentContext, currentContext.getMethod());
                return currentContext.setError(error);
            }).whenComplete((context, t) -> report(context));
        ...
        for (Future<IterationContext> future : futures) {
            future.get();
        }
    }
